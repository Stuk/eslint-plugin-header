                    ...
                    if (!leadingComments.length) {
                        ...
                        if (commentType === "line") {
                            ...
                            if (hasError) {
                                ...
                                var matchYearRangeRegexPattern = /(\d{4})(?:-(\d{4}))?/g;
                                var previousHeaderYear = incorrectHeader.match(matchYearRangeRegexPattern);
                                var settings = findSettings(options);

                                if (previousHeaderYear && settings && settings.persistYear) {
                                    fixLines = [fixLines[0].replace(matchYearRangeRegexPattern, previousHeaderYear[0])]
                                }
