        ...
        return {
            Program: function(node) {
                if (!hasHeader(context.getSourceCode().getText())) {
                    var leadingCommentsAfterHeader = getLeadingComments(context, node);
                    var firstToken = context.getFirstToken(node);

                    if (!leadingCommentsAfterHeader.length) {
                        context.report({
                            loc: node.loc,
                            message: "missing header",
                            fix: genPrependFixer(commentType, node, fixLines, eol, numNewlines)
                        });
                    } else if (!firstToken && leadingCommentsAfterHeader[0].range[0] !== 0) {
                        context.report({
                            loc: {
                                start: {
                                    line: 1,
                                    ...
                                    line: leadingCommentsAfterHeader[0].loc.start.line - 1,
                                    column: 0,
                                },
                            },
                            message: "unexpected preceding whitespace",
                            fix: function(fixer) {
                                return fixer.replaceTextRange([0, leadingCommentsAfterHeader[0].range[0]], "");
                            },
                        });
                    } else if (leadingCommentsAfterHeader[0].loc.start.line < firstToken.loc.start.line) {
                        context.report({
                            loc: {
                                start: {
                                    line: 1,
